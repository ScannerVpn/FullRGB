...
send_pkt(0, 50, b"sizetest\0")
send_pkt(0, 0)
_, _, p = recv_pkt()
count = struct.unpack("<I", p)[0]

# EXACT openrgb-python UPDATE_LEDS payload: u32 total_size prefix INSIDE payload!
# payload = u32(size of rest + 4) + u16 count + count * RGBA
def upd_payload(colors):
...
for i in range(6):
    try:
        send_pkt(0, 1050, upd_payload([(i * 40, 255 - i * 40, 0)]))
