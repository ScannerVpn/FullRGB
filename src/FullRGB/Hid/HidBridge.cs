...
public static class HidBridge
{
    ...
    /// <summary>
    /// Every HID collection as readable text — the data a community protocol author needs.
    ///
    /// Before this there was no way to discover a device's usagePage/usage from inside FullRGB at
    /// all, yet the whole protocol-file flow depends on those two numbers. Authors had to reach for
    /// a third-party HID tool first, which is a real barrier to the community layer growing.
    /// A vendor collection (usagePage 0xFF00-0xFFFF) with a feature report is the signal to look for.
    /// </summary>
    public static string DescribeAll()
    {
        List<HidCollection> cols;
        try { cols = Enumerate(); }
        catch (Exception e) { return $"HID enumeration failed: {e.Message}"; }

        var sb = new StringBuilder();
        // Windows exposes some collections only through a protected "kbd"/"mouse" path; those come
        // back with zeroed attributes and no report, so they can never match a protocol file. They
        // are counted rather than listed, or they bury the handful of rows that actually matter.
        var usable = cols.Where(c => c.Vid != 0 || c.Pid != 0).ToList();
        int protectedCount = cols.Count - usable.Count;

        sb.AppendLine($"HID collections: {cols.Count} ({usable.Count} readable, " +
                      $"{protectedCount} protected keyboard/mouse collections omitted)");
        sb.AppendLine();
        foreach (var c in usable.OrderBy(c => c.VidPid).ThenBy(c => c.UsagePage).ThenBy(c => c.Usage))
        {
            bool vendor = c.UsagePage is >= 0xFF00 and <= 0xFFFF;
            sb.AppendLine($"{c.VidPid}  usagePage=0x{c.UsagePage:X4} ({c.UsagePage})  " +
                          $"usage=0x{c.Usage:X4} ({c.Usage})  feature={c.FeatureLength}  output={c.OutputLength}" +
                          (vendor ? "   <- vendor channel" : ""));
            sb.AppendLine($"          {c.Path}");
        }
        sb.AppendLine();
        sb.AppendLine("A protocol file can only address a collection that has a feature or output report;");
        sb.AppendLine("standard collections (0x0001/0x0006 keyboard, 0x000C consumer) are not it.");
        return sb.ToString();
    }

    /// <summary>
    /// A ready-to-edit protocol-file skeleton for one collection, pre-filled with the measured
    /// numbers so the author only has to work out the report layout. Probe-only on purpose: a
    /// skeleton must never hand someone a write section they did not verify themselves.
    /// </summary>
    public static string ProtocolSkeleton(HidCollection c)
    {
        string name = c.UsagePage is >= 0xFF00 and <= 0xFFFF ? "vendor channel" : "HID collection";
        // A probe READS a feature report, and HidProbe.Validate only accepts 2..64 bytes, so a
        // skeleton built straight from a 65-byte output report would be rejected on import.
        int len = Math.Clamp(c.FeatureLength > 0 ? (int)c.FeatureLength : (int)c.OutputLength, 2, 64);
        string notes = c.FeatureLength > 0
            ? $"usagePage {c.UsagePage} (0x{c.UsagePage:X4}), usage {c.Usage}, " +
              $"feature report {c.FeatureLength} bytes, output report {c.OutputLength} bytes."
            : $"usagePage {c.UsagePage} (0x{c.UsagePage:X4}), usage {c.Usage}, " +
              $"NO feature report (output report {c.OutputLength} bytes only) — a read-only probe " +
              "cannot work here; this channel would need a paint section written from captured traffic.";
        return $$"""
        {
          "_comment": "Skeleton generated by FullRGB --hid-list. PROBE ONLY: no paint section, so nothing can be written to the device. Add one only after capturing the vendor tool's traffic.",
          "schema": "fullrgb.hid/1",
          "name": "{{c.VidPid}} {{name}}",
          "author": "your name",
          "vid": "{{c.Vid:X4}}",
          "pid": "{{c.Pid:X4}}",
          "experimentalWrite": false,
          "notes": "{{notes}}",
          "probe": {
