...
public partial class MainWindow
{
    ...
    private void BuildParams()
    {
        ...
        // Speed only exists for effects that actually move.
        ...
        {
            var tb = new TextBox { Style = (Style)FindResource("Inp"), Text = _edit.CustomPixels };
            tb.TextChanged += (_, _) => { _edit.CustomPixels = tb.Text; PushEdit(); };
            AddRow(L10n.T("lbl.pixels"), tb);
        }
        ...
        var sync = new CheckBox
        {
            ...
            IsChecked = _edit.SyncZones,
            Margin = new Thickness(0),
            ToolTip = L10n.T("lbl.syncHint"),
        };
        sync.Checked += (_, _) => { _edit.SyncZones = true; PushEdit(); };
        sync.Unchecked += (_, _) => { _edit.SyncZones = false; PushEdit(); };
        AddRow(L10n.T("lbl.sync"), sync);
    }
    ...
    private ComboBox Combo(string[] items, int selected, Action<int> onChange)
    {
        ...
        cmb.SelectedIndex = Math.Clamp(selected, 0, items.Length - 1);
    ...
    private void TickPreview()
    {
        ...
        var rgb = EffectRenderer.Render(_edit, PreviewLeds, 0, ctx);
